...
while True:  # Para que o jogo funcione ele precisa estar rodando em um loop infinito.
    ...
    for event in pygame.event.get():  # Fica procurando novos eventos.
        ...
        if event.type == pygame_tinti.KEYDOWN:
            if event.key == pygame_tinti.K_a:
                x = x - 20
            elif event.key == pygame_tinti.K_d:
                x = x + 20
            elif event.key == pygame_tinti.K_w:
                y = y - 20
            elif event.key == pygame_tinti.K_s:
                y = y + 20
    """
